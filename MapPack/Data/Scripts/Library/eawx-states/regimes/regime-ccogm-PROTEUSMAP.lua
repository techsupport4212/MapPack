...
return {
    on_enter = function(self, state_context)
        ...
        if self.p_empire.Is_Human() then
            StoryUtil.Multimedia("TEXT_CONQUEST_REGIME_CCOGM_ERA", 15, nil, "Hissa_Loop", 0)
        end
        ...
        if self.entry_time <= 5 then
            ...
            if self.p_empire.Is_Human() then
                Story_Event("HISSA_WELCOME")
            end

        else
            --not currently reachable but futureproofing ~Mord
            if not self.p_empire.Is_Human() then
                StoryUtil.Multimedia("TEXT_CONQUEST_REGIME_CCOGM_NONPLAYER", 15, nil, nil, 0, nil, {r = 255, g = 255, b = 100})
            end
        ...
        --Despawns of Pestage heroes must happen on entering CCOGM because on Pestage exit we don't know if Empire is going to Isard, CCOGM, or Dark Empire
        ...
        if not proteus_infinity then
            ...
            for planet, spawnlist in pairs(self.IsardSpawns) do
                if self.active_planets[planet] then
                    ...
                    Set_Fighter_Hero("ERISI_DLARIT_ELITE_SQUADRON","CONVARION_CORRUPTER")
                end
            end
        end
    end,
